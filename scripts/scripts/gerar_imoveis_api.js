...
async function gerarImovel(contrato, numero) {
  ...
  const endereco = enderecoTemplate.replace('{numero}', numero * 10 + Math.floor(Math.random() * 100));
...
async function main() {
  ...
  try {
    ...
    console.log(`[API] ${contratos.data.length} contratos encontrados`);
    ...
    for (const contrato of contratos.data) {
      try {
        ...
        const imovel1 = await gerarImovel({ ...contrato, tipo_contrato: 'Locacao' }, contratos.data.indexOf(contrato) + 1);
        ...
        const imovel2 = await gerarImovel({ ...contrato, tipo_contrato: 'Venda' }, contratos.data.indexOf(contrato) + 1);
